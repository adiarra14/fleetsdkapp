...
// Version 2.2 (2025-07-22) - Fixed SDK integration with database persistence and connection retry
import java.io.*;
import java.net.*;
...
public class RetryMaxvisionSdkTcpServer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("=== SDK INTEGRATION v2.2 INITIALIZING ===");
        ...
        try {
            ...
            System.out.println("=== SDK INTEGRATION v2.2 READY ===");
        } catch (Exception e) {
        ...
        executor.submit(() -> {
            try {
                while (true) {
                    ...
                    System.out.println("HEARTBEAT: Maxvision SDK TCP Server v2.2 alive at " + LocalDateTime.now());
                }
        ...
        System.out.println("Maxvision SDK TCP Server v2.2 is running successfully");
    ...
    private static void startHealthServer() {
        try {
            ...
            // Simple health endpoint that always returns 200 OK to keep container running
            // Will return 200 even if database is not connected
            httpServer.createContext("/", exchange -> {
                String response = "Maxvision SDK TCP Server v2.2 - " + LocalDateTime.now();
            ...
            // Enhanced health endpoint that returns 200 only if database is connected
            httpServer.createContext("/health", exchange -> {
                String response;
                int statusCode;
                
                if (databaseConnected) {
                    response = "Maxvision SDK TCP Server v2.2 - Healthy - DB Connected - " + LocalDateTime.now();
                    statusCode = 200;
                } else {
                    response = "Maxvision SDK TCP Server v2.2 - Unhealthy - DB Not Connected - " + LocalDateTime.now();
    ...
    private static void initDatabaseWithRetry() {
        // Start database initialization in a separate thread with retry
        executor.submit(() -> {
            int retries = 0;
            int maxRetries = 30;
            int retryDelaySeconds = 5;
            
            while (!databaseConnected && retries < maxRetries) {
                try {
                    retries++;
                    System.out.println("Attempting database connection (attempt " + retries + " of " + maxRetries + ")...");
                    ...
                    try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
                        ...
                        try (Statement stmt = conn.createStatement();
                            ...
                            // Table not found is still ok - we've connected to the database
                            databaseConnected = true;
                        }
                    }
                } catch (Exception e) {
                    System.out.println("Database connection attempt failed (" + retries + "/" + maxRetries + "): " + e.getMessage());
                    
                    if (retries >= maxRetries) {
                        System.err.println("Maximum database connection retries reached. Continuing without database.");
                    } else {
            ...
            public void handleMessage(Object message) {
                ...
                try {
                    ...
                    try {
                        ...
                        // Store data in database if we have a device ID and database is connected
                        if ((deviceId != null || ipAddress != null) && databaseConnected) {
                            storeBaliseData(dataMap);
                        } else if ((deviceId != null || ipAddress != null) && !databaseConnected) {
                            System.out.println("[SDK-v2] Skipping database storage for " + deviceId + " - database not connected");
                        }
    ...
    private static void processBaliseData(String data, String clientIP) {
        ...
        // Only store data if database is connected
        if (databaseConnected) {
            storeBaliseData(info);
        } else {
            System.out.println("[SDK-v2] Skipping database storage - database not connected");
        }
    }
    ...
    private static void storeBaliseData(Map<String, Object> data) {
        if (!databaseConnected) {
            System.out.println("[SDK-v2] Database not connected, skipping data storage");
            return;
        }
